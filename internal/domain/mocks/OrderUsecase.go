...
// GetOrderHPP provides a mock function with given fields: ctx, id
func (_m *OrderUsecase) GetOrderHPP(ctx context.Context, id string) (*domain.OrderHPPBreakdown, error) {
	ret := _m.Called(ctx, id)

	if len(ret) == 0 {
		panic("no return value specified for GetOrderHPP")
	}

	var r0 *domain.OrderHPPBreakdown
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, string) (*domain.OrderHPPBreakdown, error)); ok {
		return rf(ctx, id)
	}
	if rf, ok := ret.Get(0).(func(context.Context, string) *domain.OrderHPPBreakdown); ok {
		r0 = rf(ctx, id)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*domain.OrderHPPBreakdown)
		}
	}
